...
auto DeadLockProfiler::PushLock(const char* name) -> void
{
	...
	{
		lockId = static_cast<int32_t>(_name_to_id.size());
	...
	{
		const int32_t prevId = LLockStack.top();
		if (lockId != prevId)
		{
			std::set<int32_t>& history = _lock_history[prevId];
...
auto DeadLockProfiler::PopLock(const char* name) -> void
{
	...
	const int32_t lockId = _name_to_id[name];
...
{
	const auto lockCount = static_cast<int32_t>(_name_to_id.size());
	_discovered_order = std::vector<int32_t>(lockCount, -1);
	_discovered_count = 0;
...
auto DeadLockProfiler::Dfs(const int32_t here) -> void
{
	...
	const std::set<int32_t>& nextSet = findIt->second;
	for (int32_t there : nextSet)
	{
