...
SendBuffer::SendBuffer(const SendBufferChunkRef owner, BYTE* buffer, uint32 allocSize) :
	_owner(owner),
	_buffer(buffer),
...
auto SendBuffer::AllocSize() const -> uint32
{
	return _alloc_size;
}

auto SendBuffer::WriteSize() const -> uint32
{
	return _write_size;
}
