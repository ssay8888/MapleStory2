...
auto ViBufferRect::Create(const ComPtr<IDirect3DDevice9>& graphicDevice) -> std::shared_ptr<ViBufferRect>
{
	auto instance = std::make_shared<ViBufferRect>(graphicDevice);
...
auto ViBufferRect::Clone(void* pArg) -> std::shared_ptr<Component>
{
	...
	{
		MSGBOX("Failed to Clone VIBufferRect");
		return nullptr;
	}
	return instance;
}
