                                       ...
                                       const int32_t numTextures) -> HRESULT
{
	...
	for (int32_t i = 0; i < numTextures; ++i)
	{
		...
		{
			if (FAILED(D3DXCreateTextureFromFile(_graphic_device.Get(), szTexturePath, reinterpret_cast<LPDIRECT3DTEXTURE9*>(pTexture.Get()))))
			{
				return E_FAIL;
			}
		}
		...
		{
			if (FAILED(D3DXCreateCubeTextureFromFile(_graphic_device.Get(), szTexturePath, reinterpret_cast<LPDIRECT3DCUBETEXTURE9*>(pTexture.Get()))))
			{
				return E_FAIL;
			}
		}
