#include "c_pch.h"
#include "berserker_volcanicslash_state.h"
...
auto BerserkerVolcanicslashState::Enter() -> void
{
	...
	{
	case kAnimationType::kBerserkerVolcanicslash1A:
		_seq = kfm->seqs[gender == false ? 642 : 640];
		_player->ChangeAnimation(kAnimationType::kBerserkerVolcanicslash1A);
		break;
	case kAnimationType::kBerserkerVolcanicslash2A:
		_seq = kfm->seqs[gender == false ? 643 : 641];
		_player->ChangeAnimation(kAnimationType::kBerserkerVolcanicslash2A);
		break;
	default:
		_seq = kfm->seqs[gender == false ? 642 : 640];
		_player->ChangeAnimation(kAnimationType::kBerserkerVolcanicslash1A);
...
auto BerserkerVolcanicslashState::Tick(const double timeDelta) -> void
{
	_aabb_com->UpdateCollider();
	...
	if (_is_move)
	{
		for (auto [key, time] : _origin_moves)
		{
			...
			if (_player->GetAnimationTimeAcc() >= time && iter == _moves.end() && 
				_player->GetCurrentAnimationType() == kAnimationType::kBerserkerVolcanicslash1A)
			{
				_moves.emplace(key, time);
				const auto transform = _player->GetTransform();
				transform->SetUpRotation(_float3(0.f, 1.f, 0.f), _player->GetRadian());
				transform->BackStraight(timeDelta * 25);
...
auto BerserkerVolcanicslashState::LateTick(const double timeDelta) -> void
{
	switch (_player->GetCurrentAnimationType())
	{
	...
	case kAnimationType::kBerserkerVolcanicslash1A:
		...
		break;
	case kAnimationType::kBerserkerVolcanicslash2A:
		_player->PlayAnimation(timeDelta * 0.9);
		break;
	default:
		break;
	}
...
auto BerserkerVolcanicslashState::Render() -> void
{
