...
Coat::Coat(const ComPtr<IDirect3DDevice9>& device):
	GameObject(device)
{
}
...
auto Coat::Clone(void* arg) -> std::shared_ptr<GameObject>
{
	std::shared_ptr<Coat> pInstance = std::make_shared<Coat>(*this);
...
auto Coat::Create(const ComPtr<IDirect3DDevice9>& device) -> std::shared_ptr<Coat>
{
	std::shared_ptr<Coat> pInstance = std::make_shared<Coat>(device);
...
auto Coat::GetMesh() -> std::shared_ptr<MeshDynamic>
{
	return _mesh_com;
}

HRESULT Coat::AddComponents()
{
	...
	if (FAILED(AddComponent(kSceneCharacterSelect, TEXT("Prototype_Mesh_Coat"), TEXT("Com_Mesh_0"), reinterpret_cast<std::shared_ptr<Component>*>(&_mesh_com))))
		return E_FAIL;
...
HRESULT Coat::SetUpConstantTable()
{
